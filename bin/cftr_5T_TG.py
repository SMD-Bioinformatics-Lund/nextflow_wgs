...
def main(args):
    ...
    for var in vcf_object.fetch("7", cftr_region['start'], cftr_region["end"]):
        variants.append({
            ...
            "ref": var.ref,
            "alt": var.alts[0],
            "id": f"{var.pos}_{var.ref}_{var.alts[0]}"
        })
    if len(variants) == 0:
    ...
    if BAD_T_COUNT in [allele1,allele2]:
        variation_combo = combinatorics_to_find_T_causatives(variants,[allele1,allele2])
    else:
        ...
        for fragment in fragments_11tg:
            ...
            count[max_count] = count[max_count] +1
        allele1,allele2 = find_one_or_two_alleles(count)
        variation_combo = combinatorics_to_find_TG_causatives(variants,[allele1,allele2])
