workflow CALL_AND_ANNOTATE_STRS {
    ...
    // We only want to see the proband calls in the scout STR view and avoid
    // seeing the parental calls.
    // Hence, here we add dummy samples to the proband STR VCF so it can be
    // loaded into scout without the actual parental calls.
